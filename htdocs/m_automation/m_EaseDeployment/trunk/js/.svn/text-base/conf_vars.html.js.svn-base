...
function read_vars_from_ui() {
    ...
    $.each($("table#var_table tbody tr"), function(index, trObj) {
        var key = $(trObj).children().first().children().first().val();
        var value = $(trObj).children().first().next().children().first().val();
...
// deal with blur on td>input  失去焦点时
...
$(document).on("click", "a.copy_var", function(e) {
    var key = $(this).parent().parent().children().first().children().first().val();
    var value  = $(this).parent().parent().children().first().next().children().first().val();
...
$(document).on("change","input.var_input",function(e){
    read_vars_from_ui();
    e.preventDefault();
});
