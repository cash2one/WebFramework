    ...
    $("table#collect_detailstep_table").tableDnD({
        ...
        onDrop: function(table, row) {
            ...
            if(_opDragCollectStepStartIndex != _opDragCollectStepEndIndex){
                editCollect("collect.cmd", getCollectSteps());
...
function copyNewCollect(){
    var collectObj = Clone(_confObj.collects[_opCollectIndex]);
...
//加密密码
    $.get(_encrypt_php, {"password":_passwd},function(data){
        ...
        _deployNumber += 1;
        $.post(_web_deploy_php_file, {"user": _ldap, "password": data, "key": _key, "deployuser": _deploy_user, "filepath": filepath,"logpath": logpath,"deployname":deployname,"hostname":hostname,"type":type}, function(message) {
            setTimeout(function(){
                _deployNumber -= 1;
...
function stopDeploy(deployname,hostname,type){
    var ret_val = confirm("是否停止部署: " + deployname + " ?");
    if (ret_val == false){
	    return;
    }
...
$(document).on("click", "a.add_collect", function(e) {
    ...
    $("table#collect_detailstep_table").css('display','none');
    $("table#collect_detail_table").css('display','none');
...
function edit_collect(element,e){
    $("table#collect_detailstep_table").css('display','');
    $("table#collect_detail_table").css('display','');
    clear_collect_info();
    _opCollectType = "edit";
...
$(document).on("change", ".collect_info", function(e) {
    ...
    if ($(this).hasClass("collect_name")) {
        if(!is_valid(value)){
            alert("集合名不能含非法字符(非字母 数字 . _) 同时字符\".\" 必须在字符串中间使用 ");
            return false;
        }
        key = "collect.name";
...
$(document).on("keyup", "input.collect_name", function(e) {
    ...
    if(!is_valid(value)){
        ...
        $("tbody#collect_tbody").children().eq(_opCollectIndex).children().first().children().first().html(value);
    }
    e.preventDefault();
});
