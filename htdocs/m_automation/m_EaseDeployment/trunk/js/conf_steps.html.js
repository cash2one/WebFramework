function copyNewStep(){
    ...
    if(_opStepIndex == _confObj.steps.length - 1) {
        ...
        _stepCmdHistory.splice(_opStepIndex + 1, 0, cmdHisObj);
    }
}
...
function addNewStep() {
    ...
    if (_opStepIndex == -1) {
        ...
        _stepCmdHistory.unshift(cmdHisObj);
...
function delStep() {
   ...
   _stepCmdHistory.splice(_opStepIndex, 1);
}
...
function getStepcmd(){
    ...
    if(stepcmd.indexOf(sep)>=0) {
      stepcmd = stepcmd.substring(0,stepcmd.length-sep.length);
    }
    return stepcmd;
}

function getStepcmdSep(){
...
function setStepCmdHistory(stepcmd){
    var key = getAPIOfStep(stepcmd);
    _stepCmdHistory[_opStepIndex][key] = stepcmd;
    _stepCmdHistory[_opStepIndex][key + ".sep"] = getStepcmdSep();
}
...
function getDefaultcmd(){
    ...
    var newrow = get_step_shellcmdarg_row("shell命令cmd:","");
...
$(document).on("click", "a.copy_step", function(e) {
    $('table#step_info_table').css('display','none');
...
// create GUI for step cmd 
function create_stepcmd_GUI(stepkey, stepcmd, stepcmdsep){
    ...
    var cmdtable = $("table#step_info_cmd_table tbody");
    if(stepkey == "shell"){
        newrow = get_step_shellcmdarg_row("shell命令cmd:",stepcmd.substring(stepcmd.indexOf(":")+1));
        cmdtable.append(newrow);        
    }else{
        args = getArgs(stepcmd,sep);
...
function edit_step(element,e){
    ...
    $("table#step_info_table").css('display','');
    ...
    if(stepkey!=null){
        create_stepcmd_GUI(stepkey, stepObj["step.cmd"], stepObj["step.cmd.sep"]);
        
        //将cmd的内容记录到历史变量中
        setStepCmdHistory(getStepcmd()); 
    }
...
$(document).on("change","select#step_cmd",function(e){
    ...
    clear_step_cmd_table();
    var stepkey = $("select#step_cmd").val();
    var stepcmd = _stepCmdHistory[_opStepIndex][stepkey];
    if (stepcmd != null){
        var stepcmdsep = _stepCmdHistory[_opStepIndex][stepkey + ".sep"];
        create_stepcmd_GUI(stepkey, stepcmd, stepcmdsep); 
    }else{
...
$(document).on("change", ".step_info", function(e) {
    ...
    if ($(this).hasClass("step_name")) {
        if(!is_valid(value)){
            alert("步骤名不能含非法字符(非字母 数字 . _) 同时字符\".\" 必须在字符串中间使用 ");
            return;
        }
        key = "step.name";
...
$(document).on("keyup", "input.step_name", function(e) {
    ...
    if(!is_valid(value)){
        ...
        $("tbody#steps_tbody").children().eq(_opStepIndex).children().first().children().first().html(value);
    }
    e.preventDefault();
});
