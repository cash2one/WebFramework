...
class EXECUTE:
	def __init__(self, conf_file, username):
		global exec_file
	...
	def exec_basic_module(self, module, type):
		...
		if function == "shell":
			...
			try:
				...
				param_list = [x.strip() for x in re.split(r";|\n",parameters)]

                # 如果不是通过shell命令来部署环境，而且通过web端部署，则在svn命令后面加上用户名
				if sysio.DEPLOY_MODE != "shell":
				    for index in range(len(param_list)):
				        if "svn " in param_list[index]:
				            param_list[index] = param_list[index] + "  --username " + self.username + " --no-auth-cache"
				parameters = "\n".join(["%s" % x for x in param_list])
				# 用echo将shell脚本字符串写入到sh文件中时,会将带$的命令先执行再写入到脚本，为了避免这种情况，将$加\。
				parameters = parameters.replace("$","\$")
				cmdlist = "echo \"" + parameters + "\" > " + self.exec_file + ";sh " + self.exec_file + ";"
                
                # 通过shell命令来部署环境时，用户需要手动与linux进行交互，如：输入密码。且部分错误log会输出到console上
				if sysio.DEPLOY_MODE == "shell":
				    (output,err) = subprocess.Popen(cmdlist + "echo status$?done", shell=True, stdout=subprocess.PIPE).communicate()

                # 通过web端来部署环境时，支持自动交互,log都打印到日志中
				else:
				    (output,err) = subprocess.Popen(cmdlist + "echo status$?done", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin = subprocess.PIPE).communicate()
				m = re.match(r'((.*\n?)*?)((.*?echo )?)(status)(.*)(done)(.*\n?)',output)
				status = m.group(6)
