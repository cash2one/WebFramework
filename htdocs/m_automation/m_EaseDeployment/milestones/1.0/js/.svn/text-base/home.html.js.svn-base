...
{
	$.post(_log_php, {"linenum": _lognumber, "logpath":logname}, function(message) {
		//alert(message);
        var result = $.parseJSON(message);
        
        _lognumber =  result["linenum"];
        //alert(_lognumber);

        for(var i=0;i <result["lines"].length; i++){
            $("div#page_log").append(result["lines"][i]);
            
            $("div#page_log").append("<br/>");
        }
	});
}
...
function get_var_row(key, value) {
    return "<tr><td class='kv_td var_key'>" + key + "</td>" + 
        ...
        "<td><a href='' class='var_link add_var'>添加</a>" +
            "<a href='' class='var_link del_var'>删除</a></td></tr>";
}
...
function get_step_row(step_name) {
    return "<tr><td class='step_name'>" + step_name + "</td>" + 
        "<td><a href='' class='step_link add_step'>添加</a>" +
            ...
            "<a href='' class='step_link edit_step'>编辑</a></td></tr>";
}
