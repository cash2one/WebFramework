...
#		self.parser = parser
	...
	def exec_basic_module(self, module, type):
		...
		if function == "shell":
			...
			try:
				(output,err) = subprocess.Popen(parameters, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin = subprocess.PIPE).communicate()
				if output == "":
					output = "null"
				log.info("EXECUTE", "The result of command(%s) is %s" % (parameters,output))
			except Exception, e:
				log.error("EXECUTE","exec shell command(%s) Exception: %s" % (parameters, e))
				self.exit()
			if ignore_fail != "1" and err != "" and output == "":
				log.error("EXECUTE", "Invalid command(%s)" % parameters + "  %s" + err)
				self.exit()
		else:
			try:
				...
				if ret == False:
					if ignore_fail != 1:
						log.info("EXECUTE", "[NOTICE]: Stop deployment for failed command(%s)" % func_exec_str)
						self.exit()
					else:
