...
# def __del__(self):
 ...
 def check_disk(self,path,disk_size):
  ...
  free = int(output[0:-1])
  self.logcheckinfo('机器剩余磁盘空间: %s' % free)
  if free >= int(disk_size) :
   self.logcheckinfo('您需要的磁盘空间: %s ,目前机器剩余磁盘空间为: %s' % (disk_size,free))
   return 1
  else : 
   self.logcheckinfo('您需要的磁盘空间: %s ,目前机器剩余磁盘空间为: %s' % (disk_size,free))
   return 0  
 ...
 def check_mem(self,mem_size):
  ...
  self.logcheckinfo('系统剩余内存: %s ' % output)
  free = int(output)
  if free >= int(mem_size) :
   self.logcheckinfo('您需要的系统内存: %s, 系统剩余内存: %s' % (mem_size ,output))
   return 1
  else :
   self.logcheckinfo('您需要的系统内存: %s ,系统剩余内存: %s' % (mem_size, output))
   return 0
 ...
 def check_pname_exist(self,pname):
  ...
  self.logcheckinfo('匹配到您要查找的进程名的次数: %d' % matchsize)
  if matchsize > 0:
   return 1
  else:
   return 0
