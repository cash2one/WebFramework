...
$i = 0;
$lines = file("./conf.txt");
array_push($lines, "qt102qt102x.corp.youdao.com27027");
$cnt = count($lines);
...
for ($i = 0; $i < $cnt; $i++) {
    $line = trim($lines[$i]);
    list($name, $host, $port) = explode("", $line);
    $MONGO["servers"][$i]["mongo_name"] = "$name";//mongo server name
    // $MONGO["servers"][$i]["mongo_host"] = "127.0.0.1";//mongo host
    $MONGO["servers"][$i]["mongo_host"] = "$host";//mongo host
    // $MONGO["servers"][$i]["mongo_port"] = "27017";//mongo port
    $MONGO["servers"][$i]["mongo_port"] = "$port";//mongo port
