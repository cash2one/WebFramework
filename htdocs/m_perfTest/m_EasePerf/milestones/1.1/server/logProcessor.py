...
# 假设log的时间单调递增
class LogProcessor:
    def __init__( self, whiteList, timeInterval = 30 , startdate = None, notimestamp = False ):
        ...
        self.threshold = 3600  # day grow when find timestamp plus
        ...
        self.timePatterns = ['[^\d]*(\d{2}):(\d{2}):(\d{2})\.\d{3}.*', '[^\d]*\d{6} (\d{2})(\d{2})(\d{2}).*', '[^\d]*\d{4}-\d{2}-\d{2} (\d{2}):(\d{2}):(\d{2}).*', '[^\d]*\d{4}(\d{2})(\d{2})(\d{2}).*']
    ...
    def getStatisticData( self, logLines ):
        for line in logLines:
            ...
            if self.noTimeStamp:
                ...
                timestamp = self.time2num( line[:index] )
            if -1 == timestamp:
