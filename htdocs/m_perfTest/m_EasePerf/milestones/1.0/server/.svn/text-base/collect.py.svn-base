...
class Collect():
    ...
    def start( self ):
        if '' != self.readPid():
            print 'already running? if not, please remove file \"%s\" first' % self.pidFile
            exit( -1 )
        # logger
        self.getLogger()
...
def main( argv ):
    ...
    parser = optparse.OptionParser( usage = '''python collect.py start|stop [-c config.xml]''' )
    ...
    # start or stop
    if len( args ) == 1 and args[0] in ['start', 'stop']:
        collect = Collect( confFile, pidFile, logFile, options.alllog, options.date )
        if args[0] == 'start':
            collect.start()
...
# todo:
# 1. 静态log文件的统计 (可能要涉及processor的修改): low
# 2. proess handler 时间校正: high -- done
# 3. log打印线程名 (?): middle
# 4. 进程配置时不能只配置processPath? to discuss: high
# 5. 同一目录起多个collect进程 -> 文件名与配置文件名一致: high -- done
# 6. start时若.pid文件已存在, 可通过ps的environment information来判断collect进程是否已异常结束: low
# 7. getpid时忽略监控进程本身: middle --done
