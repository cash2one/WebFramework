...
function getQueryObject($param){
    #$queryobj = json_decode($param, true); if php version bigger than 5 ,user this line
...
function getMinMaxKeyAndSize($values){
    ...
    return array($minkey,$maxkey,$count);
}
    

//compress data
function compressIDDatas(& $userresult,$begin_ts = null,$end_ts = null,$floatprecise = 4,$reservercount = 300,$intervaltime = 30,$ignorecount = 1){
    foreach ($userresult as $keyname => $values){
        foreach ($values as $subkeyname => $subvalues){
            if("type" == $subkeyname || "_results" == $subkeyname ){
                continue;
            }
            list($minkey,$maxkey,$valuecount) = getMinMaxKeyAndSize($subvalues);
            if($valuecount > 1 ){
                #calc real intervaltime(assume the input intervaltime may not correct, maybe bigger or negtive number)
                $realintervaltime = ceil( ( $maxkey - $minkey ) / $valuecount );
                if($realintervaltime > $intervaltime && $intervaltime > 0){
                ...
                $tsgap = ceil( ( $maxkey - $minkey ) / $reservercount );
                if($tsgap < $realintervaltime){
                    $tsgap = $realintervaltime;
                }
                ...
                if(null != $bend_ts && $end_ts > $maxkey + 2 * $tsgap){
