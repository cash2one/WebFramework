...
// run expect script to get result lines
function get_diff_lines($ldap, $passwd, $url, $version1, $version2) {
    $output_file = "./svn_diff.exp";
    
    $content = file_get_contents(TEMPLATE_FILE);
    $content = str_replace('$current_file$', $output_file, $content);
    $content = str_replace('$url$', $url, $content);
    $content = str_replace('$version1$', $version1, $content);
    $content = str_replace('$version2$', $version2, $content);
...
function output_html_result($diffObjList) {
    ...
    foreach($diffObjList as $diffObj) {
        ...
        array_push($diff_content_lines, "<tr id='filename_$index'><th colspan=4 class='fileheader'>" . $file . "</th></tr>");
